...
def listar_alumnos():
    try:
        ...
        return jsonify({'alumnos': alumnos, 'message': "Alumnos listados", "Exito": True})
    
    except Exception as ex:
        return jsonify({"message": "Error BD: {}".format(ex), 'Exito': False})
...
def pagina_no_encontrada(error):
    return jsonify({"message": "Error 404: {}".format(error), 'Exito': False})
